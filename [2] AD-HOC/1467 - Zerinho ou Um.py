...
while True:
    try:
        jogadores = ['A', 'B', 'C']
        venc_1 = venc_2 = pos_zero = pos_um = 0
        entrada = str(input()).split()
        for k, v in enumerate(entrada):
            entrada[k] = int(v)
        if entrada.count(1) == 2:
            pos_zero = entrada.index(0)
            venc_1 += 1
        elif entrada.count(0) == 2:
            pos_um = entrada.index(1)
            venc_2 += 1
        if venc_1 > 0:
            print(jogadores[pos_zero])
        elif venc_2 > 0:
            print(jogadores[pos_um])
        else:
            print('*')
    except EOFError:
        break