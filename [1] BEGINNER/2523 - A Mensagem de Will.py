...
Todos estão desesperadamente procurando por ele por todos os cantos. Enquanto isso, coisas estranhas estão 
...
Além disso, há uma letra do alfabeto pintada na parede em baixo de cada lâmpada. Quando Will quer lhe enviar uma 
mensagem, ele irá (misteriosamente) piscar, uma a uma, as lâmpadas correspondentes a cada letra de sua mensagem. 
...
while True:
    try:
        alfabeto = list(str(input()).upper())
        num_lampada = int(input())
        ordem = [int(i) for i in list(str(input()).split())]
        mensagem = ''
        for i in ordem:
            mensagem += alfabeto[i-1]
        print(mensagem)
    except EOFError:
        break