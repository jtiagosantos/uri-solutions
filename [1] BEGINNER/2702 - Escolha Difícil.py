...
refeicoes = str(input()).split()
for k,v in enumerate(refeicoes):
    refeicoes[k] = int(v)
num_refeicoes = str(input()).split()
for k,v in enumerate(num_refeicoes):
    num_refeicoes[k] = int(v)
total = []
for i in range(3):
    if num_refeicoes[i] > refeicoes[i]:
        total.append(num_refeicoes[i] - refeicoes[i])
print(sum(total))
