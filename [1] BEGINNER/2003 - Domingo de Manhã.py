...
Parangaba às 8h, para irem juntos comprar na feira. Porém, muitas vezes Bino acorda muito tarde e se atrasa para o encontro com seu amigo.
...
while True:
    try:
        horario = input().split(sep=':')
        hora = int(horario[0])
        minuto = int(horario[1])
        atrasomax = int()
        #Condições para atraso = 0:
        if hora <= 7 and minuto <= 0:
                atrasomax = 0
        #Pra atraso > 0:
        else:
            hora += 1
            atrasomax = (hora - 8) * 60 + minuto
        
        if atrasomax < 0:
            atrasomax = 0
        print('Atraso maximo: {}'.format(atrasomax))
    except EOFError:
        break
